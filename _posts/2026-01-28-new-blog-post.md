...
$$ \Sigma*{\text{cl}}^n \xrightarrow{\partial_n} A^n \longrightarrow \Psi^0_n \longrightarrow \Sigma*{\text{cl}}^n[1] $$
...
$$ \partial*\bullet: \mathbb{K}(\Sigma*{\text{cl}}^\bullet) \longrightarrow \Sigma \mathbb{K}(A^\bullet) $$
...
- _Note:_ Membership in the totalization guarantees that the symbol carries the necessary coherence data to descend to the stack.
...
$$ E*2^{p,q} = H^p*{\text{cosimp}}(\Delta; \pi*q \mathbb{K}(A^\bullet)) \implies \pi*{p+q} \text{Tot } \mathbb{K}(A^\bullet) $$
